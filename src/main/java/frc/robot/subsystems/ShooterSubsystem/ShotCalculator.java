 ...
 *
 * <p>数据来源:队伍自建的 frc-shooter-sim **V3.1** 弹道优化器(w_tof=1.1,vortex-shedding 物理标定)。
 * sim 对 1.5–5.0 m 做 sweep 后拟合出三条关于距离 d(米)的二次多项式(见 sim 仓库
 * lookup_polynomial.json)。本类把这些多项式搬进机器人代码,作为上车标定前的**物理起点**。
 *
 * <h2>⚠️ 上车必须标定的两个旋钮</h2>
 *
 * <ol>
 *   <li><b>hood 角度约定映射</b>:sim 输出的是相对水平的<b>发射角</b>(56°~73°,距离越远越平);
 *       机器人 hood 编码器读的是相对机械零位(hardstop 归零)的<b>hood 角</b>(老手调表里是 0°~23°,
 *       距离越远越大)。两者关系是 {@code hoodAngle = HOOD_LAUNCH_ANGLE_AT_ZERO - launchAngle}。
 *       用老表端点 + sim 反算出 offset ≈ 74.5°~79.3°(均值 ~77°,中段比端点略低,说明两套模型形状
 *       不完全一致),所以 {@link #DEFAULT_HOOD_LAUNCH_ANGLE_AT_ZERO_DEG} 只是估计值,上车要校。
 *   <li><b>飞轮量级</b>:sim 飞轮 ≈ 24~33 RPS(1.5~5 m),而老手调表是 50~63 RPS,差近 2×。
 *       sim 的绝对值依赖未标定参数(Cd=0.5、slip=0.18、gear=1.0 都是估计),所以飞轮输出乘了一个
 *       {@link #DEFAULT_FLYWHEEL_RPS_SCALE}。若上车发现打不到/打太近,优先调这个 scale
 *       (老表暗示真实可能在 ~1.7–2.0× 附近,但请以实测为准,别盲信老表——它本身没验证过)。
 * </ol>
 *
 * <p>本类刻意做成<b>无状态纯函数</b>(不读 NetworkTables、不碰硬件),方便 JUnit 直接测。
 * 子系统持有 {@code LoggedTunableNumber} 后,把 offset / scale 当参数传进
 * {@link #calculate(double, double, double)} 即可在 dashboard 实时调。
 */
public final class ShotCalculator {
   ...
   */
  public record ShooterSetpoint(double flywheelRps, double hoodAngleDeg) {}
   ...
   * 把距离换算成 shooter 设定值。
   *
   * @param distanceMeters 机器人到 HUB 的水平距离(米),范围外夹到 [{@value #MIN_DISTANCE_M},
   *     {@value #MAX_DISTANCE_M}]
  ...
  public static ShooterSetpoint calculate(
    ...
    double hoodDeg =
        util8810.clamp(hoodLaunchAngleAtZeroDeg - launchDeg, HOOD_MIN_DEG, HOOD_MAX_DEG);
    double flywheelRps = (flywheelRpm(distanceMeters) / 60.0) * flywheelRpsScale;
    return new ShooterSetpoint(flywheelRps, hoodDeg);
  }
